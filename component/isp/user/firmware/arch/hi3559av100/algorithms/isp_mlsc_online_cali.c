...
static HI_S32 Get_Lsc_Data_Channel(HI_U16 *pBayerImg, HI_U32 *pu32GridData, ISP_MLSC_CALIBRATION_CFG_S *stLSCCaliCfg, ISP_LSC_Grid_Step_S *pstLscGridStepXY, HI_U16 u16BlcOffset, HI_U8 u8Value)
{
    ...
    if (HI_NULL == pBayerImg || HI_NULL == pu32GridData) {
        ISP_ERR_TRACE("%s: LINE: %d pBayerImg/pls_data is NULL point !\n", __FUNCTION__, __LINE__);
        return HI_FAILURE;
    }
    ...
    for (i = 0; i < grid_y_size; i++) {
        ...
        for (j = 0; j < grid_x_size; j++) {
            ...
            if (numTemp) {
                ...
                ISP_ERR_TRACE("Something goes wrong in getLSData()!");
                return HI_FAILURE;
            }
        }
    }
...
static HI_S32 LSC_GenerateGridInfo(HI_U16 *pu16Data, ISP_MLSC_CALIBRATION_CFG_S *pstLSCCaliCfg, ISP_LSC_Grid_Step_S *pstLscGridStepXY, ISP_MESH_SHADING_TABLE_S *pstLSCCaliResult)
{
    ...
    if (pstLSData == HI_NULL) {
        ISP_ERR_TRACE("pstLSData allocation fail!\n");
        return HI_FAILURE;
    }
    ...
    if (s32Ret != HI_SUCCESS) {
        ISP_ERR_TRACE("There are some errors in getLSData()!\n");
        ISP_FREE(pstLSData);
        return HI_FAILURE;
    }
                                  ...
                                  ISP_MESH_SHADING_TABLE_S *pstMLSCTable)
{
    ...
    size = sizeof(HI_U16) * pstMLSCCaliCfg->u16DstImgWidth * pstMLSCCaliCfg->u16DstImgHeight;
    s32Ret = HI_MPI_SYS_MmzAlloc(&phy_addr, (HI_VOID **)(&pu16CropRaw), "mlsc_calib", HI_NULL, size);
    if (s32Ret != HI_SUCCESS) {
    ...
    if (s32Ret == HI_FAILURE) {
        ISP_ERR_TRACE("There are some errors in LSC_GenerateGridInfo()!\n");
        goto end;
    }
    ...
    s32Ret1 = HI_MPI_SYS_MmzFree(phy_addr, (HI_VOID *)pu16CropRaw);
