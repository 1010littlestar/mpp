...
static HI_VOID FeDgStaticInit(VI_PIPE ViPipe, ISP_DG_STATIC_CFG_S *pstStaticRegCfg)
{
    return;
}
...
static HI_VOID DgRegsInitialize(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        ...
        BeDgDynaInit(ViPipe, &pstRegCfg->alg_reg_cfg[i].stDgRegCfg.stDynaRegCfg);
    ...
    {
        FeDgStaticInit(ViPipe, &pstRegCfg->alg_reg_cfg[0].stFeDgRegCfg.stStaticRegCfg);
        FeDgDynaInit(ViPipe, &pstRegCfg->alg_reg_cfg[0].stFeDgRegCfg.stDynaRegCfg);
                 ...
                 HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (pstIspCtx->linkage.defect_pixel && (hi_ext_system_dpc_static_defect_type_read(ViPipe) == 0)) {
        for (i = 0; i < pstRegCfg->cfg_num; i++) {
