...
HI_S32 FlickerCtxInit(VI_PIPE ViPipe)
{
    ...
    if (pastFlickerCtx == HI_NULL) {
        pastFlickerCtx = (ISP_FLICKER_ATTR *)ISP_MALLOC(sizeof(ISP_FLICKER_ATTR));
        if (pastFlickerCtx == HI_NULL) {
            ISP_ERR_TRACE("Isp[%d] FlickerCtx malloc memory failed!\n", ViPipe);
...
static HI_VOID FlickerFreeMem(VI_PIPE ViPipe)
{
    ...
    if ((pstFlickerCtx->bMalloc == HI_TRUE) && (pstFlickerCtx->pS32Mem != HI_NULL)) {
...
static HI_BOOL GetFlickEnable(VI_PIPE ViPipe)
{
    ...
    if (IS_WDR_MODE(pstIspCtx->sns_wdr_mode) && \
        ((pstIspCtx->block_attr.running_mode == ISP_MODE_RUNNING_OFFLINE)  || \
         (pstIspCtx->block_attr.running_mode == ISP_MODE_RUNNING_ONLINE))) {
        return HI_TRUE;
    } else {
        return HI_FALSE;
    }
...
static HI_VOID FlickerDynaRegInit(HI_U8 u8WDRMode, ISP_FLICKER_DYNA_CFG_S *pstDynaRegCfg)
{
    if (IS_LINEAR_MODE(u8WDRMode) || IS_BUILT_IN_WDR_MODE(u8WDRMode)) {
        pstDynaRegCfg->u8MergeFrm = 1;
    } else if (IS_2to1_WDR_MODE(u8WDRMode)) {
        pstDynaRegCfg->u8MergeFrm       =   2;

    } else if (IS_3to1_WDR_MODE(u8WDRMode)) {
        pstDynaRegCfg->u8MergeFrm       =   3;

    } else if (IS_4to1_WDR_MODE(u8WDRMode)) {
        pstDynaRegCfg->u8MergeFrm       =   4;
    } else {
        pstDynaRegCfg->u8MergeFrm       =   1;
    }
    ...
    pstDynaRegCfg->u32CntOverPre = 0;
    pstDynaRegCfg->u16OverThr    = HI_ISP_FLICKER_OVERTH;
    pstDynaRegCfg->u32OverCntThr = HI_ISP_FLICKER_OVERCNTTH;
    pstDynaRegCfg->bResh         = HI_TRUE;
}

static HI_VOID FlickerRegsInitialize(VI_PIPE ViPipe, isp_reg_cfg *pstRegCfg)
{
    HI_BOOL bEnable;
    HI_U8   i;
    ...
    for (i = 0; i < pstRegCfg->cfg_num; i++) {
        FlickerDynaRegInit(pstIspCtx->sns_wdr_mode, &pstRegCfg->alg_reg_cfg[i].stFlickerRegCfg.stDynaRegCfg);
        pstRegCfg->alg_reg_cfg[i].stFlickerRegCfg.bEnable = bEnable;
    }

    pstRegCfg->cfg_key.bit1FlickerCfg = 1;
}

static HI_VOID FlickerReadHWStatInfo(VI_PIPE ViPipe, isp_stat *pStatInfo)
{
    HI_U32 i;
    ...
    u32Val = pStatInfo->flicker_stat.cur_diff_gr[0]; // Diff
    ...
    u32Val = pStatInfo->flicker_stat.cur_diff_gb[0]; // Diff
    s32Val = (u32Val & 0x400000) ? (-((HI_S32)(((~u32Val) & 0x3FFFFF) + 1))) : (HI_S32)(u32Val & 0x3FFFFF);
    pstFlickerCtx->stFckStatInfo.s32CurFrameDiffSumGb = s32Val;

    pstFlickerCtx->stFckStatInfo.u32CurFrameAbsSumGr = pStatInfo->flicker_stat.cur_abs_gr[0]; // ABS
    pstFlickerCtx->stFckStatInfo.u32CurFrameAbsSumGb = pStatInfo->flicker_stat.cur_abs_gb[0]; // ABS

    pstFlickerCtx->stFckStatInfo.u32CurFrameCountOver = pStatInfo->flicker_stat.cur_cnt_over[0]; // ABS
    ...
    for (i = 0; i < pstFlickerCtx->u16FckInx; i++) {
        u32Val = pStatInfo->flicker_stat.g_mean[0][i];
...
static HI_VOID FlickerInitialize(VI_PIPE ViPipe)
{
    ...
    if (pstFlickerCtx->pS32Mem == HI_NULL) {
...
HI_S32 ISP_FlickerRun(VI_PIPE ViPipe, const HI_VOID *pStatInfo, HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    pstFlickerCtx->u32FrameNum = pstIspCtx->frame_cnt;
    ...
    FlickerReadHWStatInfo(ViPipe, (isp_stat *)pStatInfo);

    if (pstFlickerCtx->u32FrameNum) {
        FlickerDetect(ViPipe);
    }

    pstIspCtx->linkage.flicker     = (HI_BOOL)pstFlickerCtx->s32RetFlickResult;
    pstIspCtx->linkage.freq_result = (HI_U8)  pstFlickerCtx->s32RetFreqResult;
    ...
    pstDynaRegCfg = &pstRegCfg->alg_reg_cfg[0].stFlickerRegCfg.stDynaRegCfg;

    pstDynaRegCfg->u16OverThr    = pstFlickerCtx->u16OverThr;
    pstDynaRegCfg->u32OverCntThr = pstFlickerCtx->u32OverCntThr;
    pstDynaRegCfg->s16GrAvgPre   = pstFlickerCtx->s32PreFrameAvgGr;
    pstDynaRegCfg->s16GbAvgPre   = pstFlickerCtx->s32PreFrameAvgGb;
    pstDynaRegCfg->u32CntOverPre = pstFlickerCtx->stFckStatInfo.u32CurFrameCountOver;
...
HI_S32 ISP_FlickerCtrl(VI_PIPE ViPipe, HI_U32 u32Cmd, HI_VOID *pValue)
{
    ...
    switch (u32Cmd) {
        case ISP_WDR_MODE_SET :
            ...
            ISP_FlickerInit(ViPipe, (HI_VOID *)&pRegCfg->reg_cfg);
            break;
...
HI_S32 isp_alg_register_flicker(VI_PIPE ViPipe)
{
    isp_usr_ctx      *pstIspCtx = HI_NULL;
    ...
    pstAlgs->alg_type = ISP_ALG_FLICKER;
    pstAlgs->alg_func.pfn_alg_init = ISP_FlickerInit;
    pstAlgs->alg_func.pfn_alg_run  = ISP_FlickerRun;
    pstAlgs->alg_func.pfn_alg_ctrl = ISP_FlickerCtrl;
    pstAlgs->alg_func.pfn_alg_exit = ISP_FlickerExit;
