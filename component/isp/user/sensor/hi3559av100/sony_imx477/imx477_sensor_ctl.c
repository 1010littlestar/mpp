...
extern HI_U32 g_Imx477SlaveSensorModeTime[];
extern HI_U8 g_u8SensorImageMode;
extern HI_U8 g_u8Imx477LensMode;

static int g_fd[ISP_MAX_PIPE_NUM] = {[0 ... (ISP_MAX_PIPE_NUM - 1)] = -1};
static HI_BOOL g_bStandby[ISP_MAX_PIPE_NUM] = {0};
extern ISP_SNS_STATE_S      *g_pastImx477[ISP_MAX_PIPE_NUM];
...
static const HI_U16 gs_au16SensorCfgSeq[][IMX477_MODE_BUTT + 1] = {
    ...
    {0x0C, 0x0C, 0x0C, 0x0A, 0x0A, 0x0A, 0x0A, 0x0113},
    {0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x0114},
    {0x1F, 0x30, 0x18, 0x13, 0x0C, 0x11, 0x0B, 0x0342},
    {0x40, 0x20, 0xB0, 0x88, 0x1C, 0xc4, 0xC8, 0x0343}, // d4
    {0x0D, 0x08, 0x08, 0x0D, 0x04, 0x0b, 0x04, 0x0340},
    {0xAC, 0xe3, 0xA7, 0x20, 0x6A, 0xec, 0x83, 0x0341}, // c0
    {0x00, 0x00, 0x00, 0x01, 0x00, 0x02, 0x00, 0x0344}, // xStart[12:8]
    {0x00, 0x6C, 0x6C, 0xF4, 0x60, 0x10, 0x68, 0x0345}, // xStart[7:0]
    {0x00, 0x01, 0x01, 0x00, 0x01, 0x00, 0x01, 0x0346}, // yStart[12:8]
    {0x00, 0xB8, 0xB8, 0x00, 0xB8, 0x14, 0xB0, 0x0347}, // yStart[7:0]
    ...
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00E3},
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00E4},
    {0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x00FC},
    {0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x00FD},
    {0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x00FE},
    {0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x0A, 0x00FF},
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0220},
    {0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x0221},
    ...
    {0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0x0900},
    {0x11, 0x11, 0x11, 0x11, 0x22, 0x11, 0x22, 0x0901},
    {0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x0902},
    {0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x3140},
    ...
    {0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x3F0D},
    {0x07, 0x07, 0x07, 0x07, 0x00, 0x07, 0x00, 0x5748},
    ...
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x0401},
    ...
    {0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x0310},
...
{
    imx477_write_register(ViPipe, 0x100, 0x00);
...
{
    imx477_write_register(ViPipe, 0x100, 0x01);
    g_bStandby[ViPipe] = HI_FALSE;
    return;
}

void imx477_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip)
{
    switch (eSnsMirrorFlip) {
        default:
        case ISP_SNS_NORMAL:
            imx477_write_register(ViPipe, 0x101, 0x0);
            break;

        case ISP_SNS_MIRROR:
            imx477_write_register(ViPipe, 0x101, 0x01);
            break;

        case ISP_SNS_FLIP:
            imx477_write_register(ViPipe, 0x101, 0x02);
            break;

        case ISP_SNS_MIRROR_FLIP:
            imx477_write_register(ViPipe, 0x101, 0x03);
            break;
    }
...
void imx477_init(VI_PIPE ViPipe)
{
    ...
    u8ImgMode = g_pastImx477[ViPipe]->u8ImgMode;
    u32SlaveModeTime = g_Imx477SlaveSensorModeTime[ViPipe];
    ...
    gstImx477Sync[ViPipe].u32SlaveModeTime  = u32SlaveModeTime;
    ...
    if ((!g_bStandby[ViPipe]) || (!g_pastImx477[ViPipe]->bInit)) {
    ...
    gstImx477Sync[ViPipe].u32HsTime = (HI_U32)g_astImx477ModeTbl[u8ImgMode].f32InckPerHs;
    if (g_pastImx477[ViPipe]->astRegsInfo[0].stSlvSync.u32SlaveVsTime == 0) {
        gstImx477Sync[ViPipe].u32VsTime = g_astImx477ModeTbl[u8ImgMode].u32InckPerVs;
    } else {
        gstImx477Sync[ViPipe].u32VsTime = g_pastImx477[ViPipe]->astRegsInfo[0].stSlvSync.u32SlaveVsTime;
    }

    gstImx477Sync[ViPipe].unCfg.u32Bytes = 0xc0030000;
    gstImx477Sync[ViPipe].u32HsCyc = 0x3;
    gstImx477Sync[ViPipe].u32VsCyc = 0x3;

    // slave open,master close
    CHECK_RET(HI_MPI_ISP_SetSnsSlaveAttr(SlaveDev, &gstImx477Sync[ViPipe]));

    for (i = 0; i < g_pastImx477[ViPipe]->astRegsInfo[0].u32RegNum; i++) {
        imx477_write_register(ViPipe, g_pastImx477[ViPipe]->astRegsInfo[0].astI2cData[i].u32RegAddr, g_pastImx477[ViPipe]->astRegsInfo[0].astI2cData[i].u32Data);
    }
    imx477_restart(ViPipe);

    printf("IMX477 %s init succuss!\n", g_astImx477ModeTbl[u8ImgMode].pszModeName);

    g_pastImx477[ViPipe]->bInit = HI_TRUE;
