    ...
    switch (pstSnsState->enWDRMode) {
        ...
        case WDR_MODE_3To1_LINE:
            ...
            if (genFSWDRMode[ViPipe] == ISP_FSWDR_LONG_FRAME_MODE) {
                ...
                pstAeSnsDft->u32MaxISPDgainTarget = 65535;
                pstAeSnsDft->u8AeCompensation = 24;
                pstAeSnsDft->enAeExpMode = AE_EXP_LOWLIGHT_PRIOR;
    ...
    switch (pstSnsState->u8ImgMode) {
        case IMX290_SENSOR_1080P_30FPS_2t1_WDR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 16.5)) {
                u32VMAX = IMX290_VMAX_1080P60TO30_WDR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
                pstAeSnsDft->u32LinesPer500ms = IMX290_VMAX_1080P60TO30_WDR*30;
            } else {
        ...
        case IMX290_SENSOR_1080P_30FPS_3t1_WDR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 16.5)) {
                u32VMAX = IMX290_VMAX_1080P120TO30_WDR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
                pstAeSnsDft->u32LinesPer500ms = IMX290_VMAX_1080P120TO30_WDR*60;
            } else {
                ISP_ERR_TRACE("Not support Fps: %f\n", f32Fps);
                return;
            }
        ...
        case IMX290_SENSOR_1080P_30FPS_2t1_FRAME_WDR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 0.5)) {
                u32VMAX = IMX290_VMAX_1080P30_LINEAR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
                pstAeSnsDft->u32LinesPer500ms = IMX290_VMAX_1080P30_LINEAR*15;
            } else {
                ISP_ERR_TRACE("Not support Fps: %f\n", f32Fps);
                return;
            }
        ...
        case IMX290_SENSOR_720P_30FPS_2t1_WDR_MODE:
            if ((f32Fps <= 30) && (f32Fps >= 0.5)) {
                u32VMAX = IMX290_VMAX_720P60TO30_WDR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
                pstAeSnsDft->u32LinesPer500ms = IMX290_VMAX_720P60TO30_WDR*30;
            } else {
                ISP_ERR_TRACE("Not support Fps: %f\n", f32Fps);
                return;
            }
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        g_astimx290State[ViPipe].u32RHS1_MAX = pstSnsState->au32FL[0] - g_astimx290State[ViPipe].u32BRL * 2 - 21;
    } else if (pstSnsState->enWDRMode == WDR_MODE_3To1_LINE) {
        ...
        g_astimx290State[ViPipe].u32RHS2_MAX = pstSnsState->au32FL[0] - g_astimx290State[ViPipe].u32BRL * 3 - 25;
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        if (bFirst[ViPipe]) { /* short exposure */
            ...
            if (pstSnsState->u8ImgMode == IMX290_SENSOR_1080P_30FPS_2t1_WDR_MODE) {
        ...
        if (u8Count[ViPipe] == 0) {      /* short exposure */
            pstSnsState->au32WDRIntTime[0] = u32IntTime;
            u32ShortIntTime1[ViPipe] = u32IntTime;
            u8Count[ViPipe]++;
        } else if (u8Count[ViPipe] == 1) { /* short short exposure */
            pstSnsState->au32WDRIntTime[1] = u32IntTime;
            ...
            u32LongIntTime[ViPipe] = u32IntTime;
            u32SHS3[ViPipe] = pstSnsState->au32FL[1] - u32LongIntTime[ViPipe] - 1;
            ...
            pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = (u32RHS2[ViPipe] & 0xFF);
            pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = ((u32RHS2[ViPipe] & 0xFF00) >> 8);
            pstSnsState->astRegsInfo[0].astI2cData[16].u32Data = ((u32RHS2[ViPipe] & 0xF0000) >> 16);

            pstSnsState->astRegsInfo[0].astI2cData[17].u32Data = (u32SHS3[ViPipe] & 0xFF);
            pstSnsState->astRegsInfo[0].astI2cData[18].u32Data = ((u32SHS3[ViPipe] & 0xFF00) >> 8);
            pstSnsState->astRegsInfo[0].astI2cData[19].u32Data = ((u32SHS3[ViPipe] & 0xF0000) >> 16);

            pstSnsState->astRegsInfo[0].astI2cData[20].u32Data = (u32YOUTSIZE & 0xFF);
            pstSnsState->astRegsInfo[0].astI2cData[21].u32Data = ((u32YOUTSIZE & 0x1F00) >> 8);
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME) {
        ...
        */
        if (genFSWDRMode[ViPipe] == ISP_FSWDR_LONG_FRAME_MODE) {
            ...
            if (pstSnsState->au32WDRIntTime[0] == 2 && au32Ratio[0] == 0x40) {
                u32IntTimeMaxTmp = pstSnsState->au32FL[1] - 6 - pstSnsState->au32WDRIntTime[0];
            } else {
                ...
                u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
                u32IntTimeMaxTmp0 = pstSnsState->au32FL[0] - 6 - (g_astimx290State[ViPipe].u32RHS1_MAX - 3);
...
static HI_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ...
    switch (pstSnsState->enWDRMode) {
        ...
        case WDR_MODE_3To1_LINE:
            ...
            pstDef->pstSharpen               = &g_stIspYuvSharpenWdr;
            pstDef->unKey.bit1Drc            = 1;
            pstDef->pstDrc                   = &g_stIspDRC3To1WDR;
            pstDef->unKey.bit1Gamma          = 1;
            pstDef->pstGamma                 = &g_stIspGammaFSWDR;
            pstDef->unKey.bit1PreGamma       = 0;
            pstDef->pstPreGamma              = &g_stPreGamma;
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        pstDef->stWdrSwitchAttr.au32ExpRatio[1] = 0x200;
    }
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        if ((pstSnsState->u8ImgMode == IMX290_SENSOR_1080P_30FPS_LINEAR_MODE) || \
            (pstSnsState->u8ImgMode == IMX290_SENSOR_1080P_30FPS_2t1_FRAME_WDR_MODE)) {
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        ...
        imx290_write_register(ViPipe, IMX290_VMAX_ADDR, pstSnsState->u32FLStd & 0xFF);
        imx290_write_register(ViPipe, IMX290_VMAX_ADDR + 1, (pstSnsState->u32FLStd & 0xFF00) >> 8);
        imx290_write_register(ViPipe, IMX290_VMAX_ADDR + 2, (pstSnsState->u32FLStd & 0xF0000) >> 16);
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunImx290BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = imx290_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = imx290_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = imx290_data_byte;
        }

        // Linear Mode Regs
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = IMX290_SHS1_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = IMX290_SHS1_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = IMX290_SHS1_ADDR + 2;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;       // make shutter and gain effective at the same time
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = IMX290_GAIN_ADDR;  // gain
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 1;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = IMX290_HCG_ADDR;

        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX290_VMAX_ADDR;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX290_VMAX_ADDR + 1;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX290_VMAX_ADDR + 2;

        if (pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME) {
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX290_SHS1_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX290_SHS1_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX290_SHS1_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = IMX290_VMAX_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = IMX290_VMAX_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = IMX290_VMAX_ADDR + 2;
        ...
        // DOL 2t1 Mode Regs
            ...
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX290_SHS2_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX290_SHS2_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX290_SHS2_ADDR + 2;
            ...
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = IMX290_RHS1_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr = IMX290_RHS1_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr = IMX290_RHS1_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr = IMX290_Y_OUT_SIZE_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr = IMX290_Y_OUT_SIZE_ADDR + 1;
        ...
        // DOL 3t1 Mode Regs
            ...
            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr = IMX290_RHS2_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr = IMX290_RHS2_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[16].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[16].u32RegAddr = IMX290_RHS2_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[17].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[17].u32RegAddr = IMX290_SHS3_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[18].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[18].u32RegAddr = IMX290_SHS3_ADDR + 1;
            pstSnsState->astRegsInfo[0].astI2cData[19].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[19].u32RegAddr = IMX290_SHS3_ADDR + 2;

            pstSnsState->astRegsInfo[0].astI2cData[20].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[20].u32RegAddr = IMX290_Y_OUT_SIZE_ADDR;
            pstSnsState->astRegsInfo[0].astI2cData[21].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[21].u32RegAddr = IMX290_Y_OUT_SIZE_ADDR + 1;
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (pstSensorImageMode->f32Fps <= 30) {
        if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
            ...
            if (IMX290_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                u8SensorImageMode              = IMX290_SENSOR_1080P_30FPS_2t1_FRAME_WDR_MODE;
                pstSnsState->u32FLStd          = IMX290_VMAX_1080P30_LINEAR;
                g_astimx290State[ViPipe].u8Hcg = 0x2;
            } else {
                IMX290_ERR_MODE_PRINT(pstSensorImageMode, pstSnsState);
            ...
            if (IMX290_RES_IS_720P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                ...
                pstSnsState->u32FLStd           = IMX290_VMAX_720P60TO30_WDR * 2;
                ...
                pstSnsState->u32FLStd           = IMX290_VMAX_1080P60TO30_WDR * 2;
            ...
            if (IMX290_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                ...
                pstSnsState->u32FLStd           = IMX290_VMAX_1080P120TO30_WDR * 4;
                g_astimx290State[ViPipe].u32BRL = 1109;
...
static HI_S32 imx290_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
