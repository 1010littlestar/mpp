...
HI_U32 g_Imx334SalveSensorModeTime[ISP_MAX_PIPE_NUM] = {0, 0, 0, 0, 0, 0, 0, 0};

ISP_SLAVE_SNS_SYNC_S  gstImx334Sync[ISP_MAX_PIPE_NUM];

ISP_SNS_STATE_S *g_pastImx334Slave[ISP_MAX_PIPE_NUM] = {HI_NULL};

#define IMX334SLAVE_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastImx334Slave[dev])
#define IMX334SLAVE_SENSOR_SET_CTX(dev, pstCtx)   (g_pastImx334Slave[dev] = pstCtx)
#define IMX334SLAVE_SENSOR_RESET_CTX(dev)         (g_pastImx334Slave[dev] = HI_NULL)
...
#define IMX334_SLAVE_RES_IS_4K30_12BIT(w, h)    ((w == 3840) && (h == 2160))
...
static HI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    IMX334SLAVE_SENSOR_GET_CTX(ViPipe, pstSnsState);
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    u32MaxFps = g_astImx334ModeTbl[pstSnsState->u8ImgMode].f32MaxFps;

    switch (pstSnsState->u8ImgMode) {
        case IMX334_8M30FPS_LINER_MODE:
            if ((f32Fps <= u32MaxFps) && (f32Fps >= 0.07)) {
                u32Lines = (g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32VertiLines) * u32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
                gstImx334Sync[ViPipe].u32VsTime = (g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32InckPerVs) * (u32MaxFps / DIV_0_TO_1_FLOAT(f32Fps));
                pstAeSnsDft->u32LinesPer500ms = g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32VertiLines*u32MaxFps/2;
            } else {
        ...
        case IMX334_8M30FPS_WDR_MODE:
            if ((f32Fps <= u32MaxFps) && (f32Fps >= 0.07)) {
                u32Lines = (g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32VertiLines) * u32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
                gstImx334Sync[ViPipe].u32VsTime = (g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32InckPerVs) * (u32MaxFps / DIV_0_TO_1_FLOAT(f32Fps));
                pstAeSnsDft->u32LinesPer500ms = g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32VertiLines*u32MaxFps;
            } else {
                ISP_ERR_TRACE("Not support Fps: %f\n", f32Fps);
                return;
            }
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        g_astimx334SlaveState[ViPipe].u32deltaRHS1 = u32Lines * 2 - g_astimx334SlaveState[ViPipe].u32BRL * 2 - 2;
    } else {
        pstSnsState->u32FLStd = u32Lines;
    }

    pstSnsState->astRegsInfo[0].stSlvSync.u32SlaveVsTime = gstImx334Sync[ViPipe].u32VsTime;
...
static HI_VOID cmos_slow_framerate_set(VI_PIPE ViPipe, HI_U32 u32FullLines, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        gstImx334Sync[ViPipe].u32VsTime =  MIN((HI_U64)g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32InckPerHs * u32FullLines, IMX334_VS_TIME_MAX);
        pstSnsState->astRegsInfo[0].stSlvSync.u32SlaveVsTime = gstImx334Sync[ViPipe].u32VsTime;
        pstSnsState->au32FL[0] = (u32FullLines >> 1) << 1;
        g_astimx334SlaveState[ViPipe].u32RHS1_MAX = 2 * 2200;
        g_astimx334SlaveState[ViPipe].u32deltaRHS1 = u32FullLines - g_astimx334SlaveState[ViPipe].u32BRL * 2 - 2;
    } else {
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        /* long exposure */
            ...
            u32SHR0[ViPipe] = pstSnsState->au32FL[1] - u32LongIntTime[ViPipe] + (u32LongIntTime[ViPipe] % 2) - 2;
            ...
            if (u32RHS1[ViPipe] >= u32LastRHS1[ViPipe]) {
                ...
                if (u32deltaRHS1[ViPipe] <= g_astimx334SlaveState[ViPipe].u32deltaRHS1) {
                } else {
                    u32SHR1[ViPipe] = 9 - (u32deltaRHS1[ViPipe] - g_astimx334SlaveState[ViPipe].u32deltaRHS1) % 4
                                      + (u32deltaRHS1[ViPipe] - g_astimx334SlaveState[ViPipe].u32deltaRHS1);
                    u32RHS1[ViPipe] = u32LastRHS1[ViPipe] - (g_astimx334SlaveState[ViPipe].u32deltaRHS1 - g_astimx334SlaveState[ViPipe].u32deltaRHS1 % 4);
                }
        ...
        u32Value = MIN(MAX(u32Value, 8), pstSnsState->au32FL[0] - 5);
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        if (pstSnsState->u8ImgMode == IMX334_8M30FPS_LINER_MODE) {
            gstImx334Sync[ViPipe].u32VsTime = (g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32InckPerVs) * g_astImx334ModeTbl[pstSnsState->u8ImgMode].f32MaxFps / 5;
            u32FullLines_5Fps = (g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32VertiLines) * g_astImx334ModeTbl[pstSnsState->u8ImgMode].f32MaxFps / 5;
        } else {
            return;
        }
    }
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        ...
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > IMX334_SLAVE_FULL_LINES_MAX) ? IMX334_SLAVE_FULL_LINES_MAX : pstSnsState->u32FLStd;

        gstImx334Sync[ViPipe].u32VsTime = (g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32InckPerVs);

        imx334slave_write_register(ViPipe, IMX334_SLAVE_VMAX_L, (pstSnsState->u32FLStd & 0xff));
        imx334slave_write_register(ViPipe, IMX334_SLAVE_VMAX_M, (pstSnsState->u32FLStd & 0xff00) >> 8);
        imx334slave_write_register(ViPipe, IMX334_SLAVE_VMAX_H, (pstSnsState->u32FLStd & 0xf0000) >> 16);
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == HI_FALSE) || (pstSnsRegsInfo->bConfig == HI_FALSE)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunImx334SlaveBusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = imx334slave_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = imx334slave_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = imx334slave_data_byte;
        }

        // SHR0 related
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = IMX334_SLAVE_SHR0_L;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = IMX334_SLAVE_SHR0_M;
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = IMX334_SLAVE_SHR0_H;

        // gain related
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = IMX334_SLAVE_PGC_L;
        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = IMX334_SLAVE_PGC_H;
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;

        // Vmax
        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX334_SLAVE_VMAX_L;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX334_SLAVE_VMAX_M;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX334_SLAVE_VMAX_H;
        ...
        if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX334_SLAVE_SHR1_L;
            pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX334_SLAVE_SHR1_M;
            pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
            pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX334_SLAVE_SHR1_H;

            pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr = IMX334_SLAVE_VMAX_L;
            pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = IMX334_SLAVE_VMAX_M;
            pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = IMX334_SLAVE_VMAX_H;

            pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = IMX334_SLAVE_RHS1_L;
            pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr = IMX334_SLAVE_RHS1_M;
            pstSnsState->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr = IMX334_SLAVE_RHS1_H;

            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[14].u32RegAddr = IMX334_Y_OUTSIZE_SLAVE_L;
            pstSnsState->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 1;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr = IMX334_Y_OUTSIZE_SLAVE_H;
        ...
        pstSnsState->astRegsInfo[0].stSlvSync.u32SlaveBindDev = g_Imx334SlaveBindDev[ViPipe];
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    f32maxFps = g_astImx334ModeTbl[pstSnsState->u8ImgMode].f32MaxFps;
    pstSnsState->bSyncInit = HI_FALSE;
    ...
    if (f32Fps <= f32maxFps) {
        if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
            if (IMX334_SLAVE_RES_IS_4K30_12BIT(u32W, u32H)) {
                ...
                pstSnsState->u32FLStd = g_astImx334ModeTbl[u8SensorImageMode].u32VertiLines;
            } else {
            ...
            if (IMX334_SLAVE_RES_IS_4K30_12BIT(u32W, u32H)) {
                ...
                pstSnsState->u32FLStd                 = g_astImx334ModeTbl[u8SensorImageMode].u32VertiLines * 2;
            } else {
                ISP_ERR_TRACE("Not support! Width:%d, Height:%d, Fps:%f, WDRMode:%d\n",
                          pstSensorImageMode->u16Width,
                          pstSensorImageMode->u16Height,
...
static HI_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ...
    pstSnsState->bSyncInit = HI_FALSE;
    pstSnsState->u8ImgMode = IMX334_8M30FPS_LINER_MODE;
    pstSnsState->enWDRMode = WDR_MODE_NONE;
    pstSnsState->u32FLStd =  g_astImx334ModeTbl[pstSnsState->u8ImgMode].u32VertiLines;
    pstSnsState->au32FL[0] = pstSnsState->u32FLStd;
...
static HI_S32 imx334slave_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static HI_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ...
    IMX334SLAVE_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
    ...
    IMX334SLAVE_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static HI_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
    ...
    IMX334SLAVE_SENSOR_RESET_CTX(ViPipe);
}
