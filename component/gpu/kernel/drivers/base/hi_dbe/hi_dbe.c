...
2017/10/30                 00270170                 Modify file to support linux-4.9
******************************************************************************/
...
static int hidbe_check_mmz_phyaddr(unsigned long long u64PhyAddr, unsigned long long u64Len)
{
    hil_mmb_t *   mmb        = NULL;
...
static void hidbe_mmap_close(struct vm_area_struct *pdbe_vma)
{
	...
	if(pdbe_alloc->dbe_cpu_mappings <= 0){
		dev_err(s_dbe_device.this_device, "%s: dbe_cpu_mappings is invalid", __func__);
		return;
	}
...
static int hidbe_mmap_fault(struct vm_area_struct *pdbe_vma, struct vm_fault *pdbe_vmf)
{
	...
	if(pdbe_pageptr == NULL){
		dev_err(s_dbe_device.this_device, "%s: pdbe_pageptr is NULL", __func__);
		return -EINVAL;
	}
...
static int hidbe_mmap(struct dma_buf *pdbe_dmabuf, struct vm_area_struct *pdbe_vma)
{
	...
	size = pdbe_vma->vm_end - pdbe_vma->vm_start;
	if (size == 0) {
		dev_err(s_dbe_device.this_device, "%s: vma mmap size is invalid", __func__);
		return -EINVAL;
	}

	if (!((pdbe_vma->vm_pgoff + (size >> PAGE_SHIFT)) <= (1 + (PHYS_MASK >> PAGE_SHIFT)))) {
		dev_err(s_dbe_device.this_device, "%s: vma mmap size is invalid", __func__);
		return -EINVAL;
	}

	pdbe_vma->vm_page_prot =  pgprot_writecombine(pdbe_vma->vm_page_prot);
	ret = remap_pfn_range(pdbe_vma, pdbe_vma->vm_start, PFN_DOWN(pdbe_alloc->dbe_contig_dma_addr)
		+ pdbe_vma->vm_pgoff, size, pdbe_vma->vm_page_prot);
...
static int hidbe_ioctl_wrap_func(struct hidbe_ioctl_wrap __user *pdbe_buf)
{
	...
	/* check the mmz addr is valid */
	if(0 != hidbe_check_mmz_phyaddr(dbe_alloc_req.dbe_phyaddr, dbe_alloc_req.dbe_size))
	{
		dev_err(s_dbe_device.this_device, "%s: physical address is invalid", __func__);
